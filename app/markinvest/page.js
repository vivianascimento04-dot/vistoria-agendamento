...
export default function Home() {
  ...
  const [form, setForm] = useState({ nome:'', cpf:'', email:'', telefone:'', torre:'', apartamento:'', nomeAcompanhante:'', cpfAcompanhante:'' })
  ...
  useEffect(() => {
    if (empreendimentoSel) carregarDiasCheios(ano, mes)
  ...
  async function carregarDiasCheios(a, m) {
    if (!empreendimentoSel) return
    try {
      const mesStr = a + '-' + String(m + 1).padStart(2, '0')
      const res = await fetch('/api/horarios?mes=' + mesStr + '&empreendimento=' + encodeURIComponent(empreendimentoSel))
  ...
  async function selecionarData(ds) {
    ...
    try {
      const res = await fetch('/api/horarios?data=' + ds + '&empreendimento=' + encodeURIComponent(empreendimentoSel))
      const data = await res.json()
  ...
  async function confirmar() {
    ...
    const { nome, cpf, email, telefone, torre, apartamento, nomeAcompanhante, cpfAcompanhante } = form
    if (!nome || !cpf || !email || !telefone || !torre || !apartamento || !nomeAcompanhante || !cpfAcompanhante) {
    ...
    try {
      ...
      if (res.ok) { setEtapa(4) }
        ...
        if (data.error && (data.error.includes('ja esta confirmado') || data.error.includes('Relacionamento'))) setErro('Nao e possivel realizar agendamento para o CPF informado pois ja existe um agendamento ativo. Entre em contato com o Relacionamento.')
  ...
  return (
    <main style={{minHeight:'100vh', background:'#f0f3fa', fontFamily:"'Segoe UI',sans-serif", margin:0, padding:0}}>
      ...
      <div style={{maxWidth:'900px', margin:'0 auto', padding:isMobile?'1rem':'2rem 1rem'}}>
        ...
        {/* ETAPA 1 — SELECIONAR EMPREENDIMENTO */}
        {etapa === 1 && (
          <div style={{maxWidth:'560px', margin:'0 auto'}}>
            <div style={{background:'#fff', borderRadius:'16px', padding:'1.5rem', boxShadow:'0 8px 32px rgba(27,47,126,0.10)'}}>
              ...
              <p style={{fontSize:'13px', color:'#6b7280', margin:'0 0 1.25rem', lineHeight:'1.6'}}>O calendário exibirá apenas os horários disponíveis para o empreendimento selecionado.</p>
              <div style={{display:'flex', flexDirection:'column', gap:'8px'}}>
                {empreendimentos.length === 0 && <p style={{color:'#9ca3af', fontSize:'13px', textAlign:'center', padding:'2rem'}}>Nenhum empreendimento disponível.</p>}
                {empreendimentos.map(emp => (
                  <button key={emp} onClick={() => selecionarEmpreendimento(emp)}
                    style={{display:'flex', alignItems:'center', gap:'14px', padding:'14px 16px', borderRadius:'12px', border:'1.5px solid #e0e5f5', background:'#f8f9ff', cursor:'pointer', textAlign:'left', transition:'all 0.15s', width:'100%'}}
                    ...
                    <div style={{width:'44px', height:'44px', borderRadius:'12px', background:AZUL, display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontSize:'18px', fontWeight:'700', flexShrink:0}}>
                      {emp.charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <div style={{fontSize:'14px', fontWeight:'700', color:AZUL}}>{emp}</div>
                      <div style={{fontSize:'12px', color:'#9ca3af', marginTop:'2px'}}>Agenda independente</div>
                    </div>
        ...
        {etapa === 2 && (
          <div>
            <div style={{background:AZUL_CLARO, border:'1px solid #c0c9e8', borderRadius:'10px', padding:'10px 14px', marginBottom:'1rem', display:'flex', alignItems:'center', justifyContent:'space-between'}}>
              <div style={{display:'flex', alignItems:'center', gap:'8px'}}>
                <div style={{width:'28px', height:'28px', borderRadius:'8px', background:AZUL, display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontSize:'13px', fontWeight:'700'}}>
                  {empreendimentoSel.charAt(0)}
                </div>
                <span style={{fontSize:'13px', fontWeight:'700', color:AZUL}}>{empreendimentoSel}</span>
              </div>
              <button onClick={() => { setEtapa(1); setEmpreendimentoSel(''); setDataSel(null); setHorarios([]) }}
                style={{fontSize:'12px', color:'#5a6fa8', background:'none', border:'none', cursor:'pointer', textDecoration:'underline', fontWeight:'600'}}>Alterar</button>
            </div>
            ...
            <div style={{display:'grid', gridTemplateColumns:(!isMobile&&horarios.length>0&&!mesBloqueado)?'1fr 1fr':'1fr', gap:'1.25rem', alignItems:'start', justifyContent:'center'}}>
              <div style={{maxWidth:'420px', width:'100%', margin:'0 auto'}}>
                <div style={{background:'linear-gradient(135deg, #1B2F7E 0%, #2a45b0 100%)', borderRadius:'16px 16px 0 0', padding:'1.25rem 1.5rem', display:'flex', alignItems:'center', justifyContent:'space-between'}}>
            ...
            <div style={{background:'#f8f9ff', border:'1px solid #e0e5f5', borderRadius:'12px', padding:'1.25rem', textAlign:'left', marginBottom:'1.5rem'}}>
              ...
              <div style={{display:'flex', flexDirection:'column', gap:'8px'}}>
                <div style={{display:'flex', justifyContent:'space-between', fontSize:'13px'}}>
                  <span style={{color:'#6b7280', fontWeight:'600', textTransform:'uppercase', fontSize:'11px'}}>Empreendimento</span>
                  <span style={{fontWeight:'700', color:AZUL}}>{empreendimentoSel}</span>
                </div>
                <div style={{display:'flex', justifyContent:'space-between', fontSize:'13px'}}>

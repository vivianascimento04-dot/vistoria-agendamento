    ...
    <main style={{minHeight:'100vh', background:'#f4f6fb', fontFamily:"'Segoe UI',sans-serif"}}>
      ...
      <div style={{maxWidth:'960px', margin:'0 auto', padding:'1.5rem 1rem'}}>
        ...
        {abaAtiva === 'empreendimentos' && (
          <div>
            <div style={{background:'#fff', borderRadius:'12px', padding:'1.5rem', boxShadow:'0 1px 4px rgba(27,47,126,0.08)', marginBottom:'1rem'}}>
              <h2 style={{fontSize:'16px', fontWeight:'700', color:'#1B2F7E', margin:'0 0 16px'}}>Gerenciar Empreendimentos</h2>
              <p style={{fontSize:'13px', color:'#6b7280', margin:'0 0 16px', lineHeight:'1.6'}}>Os empreendimentos cadastrados aqui aparecerao na lista suspensa para os clientes ao realizar o agendamento.</p>
              <div style={{display:'flex', gap:'10px', marginBottom:'12px'}}>
                ...
                <button onClick={adicionarEmpreendimento} disabled={salvandoEmp||!novoEmp.trim()} style={{padding:'10px 20px', background:salvandoEmp||!novoEmp.trim()?'#9ca3af':'#1B2F7E', color:'#fff', border:'none', borderRadius:'8px', fontSize:'13px', fontWeight:'700', cursor:salvandoEmp||!novoEmp.trim()?'not-allowed':'pointer'}}>
                  {salvandoEmp?'SALVANDO...':'ADICIONAR'}
                </button>
              </div>
              {erroEmp && <p style={{color:'#dc2626', fontSize:'13px', margin:'0 0 12px'}}>{erroEmp}</p>}
              <div style={{display:'flex', flexDirection:'column', gap:'8px'}}>
                {empreendimentos.map(emp => (
                  <div key={emp} style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 14px', background:'#f8f9ff', borderRadius:'8px', border:'1px solid #e0e5f5'}}>
                    <span style={{fontSize:'14px', fontWeight:'600', color:'#1B2F7E'}}>&#127961; {emp}</span>
                    <button onClick={() => removerEmpreendimento(emp)} style={{padding:'4px 12px', background:'none', border:'1px solid #fca5a5', borderRadius:'6px', fontSize:'12px', color:'#dc2626', cursor:'pointer', fontWeight:'600'}}>REMOVER</button>
                  </div>
                ))}
                {empreendimentos.length === 0 && <p style={{color:'#9ca3af', fontSize:'13px', textAlign:'center', padding:'1rem'}}>Nenhum empreendimento cadastrado.</p>}
              </div>
            </div>
          </div>
        )}

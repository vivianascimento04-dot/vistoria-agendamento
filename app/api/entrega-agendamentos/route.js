...
export async function GET(request) {
  ...
  let query = supabase.from('entrega_agendamentos').select('*').order('data').order('horario')
...
export async function POST(request) {
  ...
  const { nome, cpf, email, telefone, empreendimento, unidade, data, horario, token } = body

  if (!nome || !cpf || !email || !telefone || !empreendimento || !unidade || !data || !horario) {
    return NextResponse.json({ error: 'Preencha todos os campos.' }, { status: 400 })
  }

  const cpfLimpo = cpf.replace(/\D/g, '')

  // Verificar CPF autorizado
  ...
  if (!autorizado) {
    return NextResponse.json({ error: 'CPF nao autorizado para entrega de chaves.' }, { status: 403 })
  }

  // Verificar se CPF ja agendou
  ...
  if (jaAgendou) {
    return NextResponse.json({ error: 'Ja existe um agendamento ativo para este CPF.' }, { status: 409 })
  }

  // Verificar vagas no horario
  ...
  if (vagas && vagas.length >= 4) {
    return NextResponse.json({ error: 'Horario lotado. Escolha outro horario.' }, { status: 409 })
  }

  // Salvar agendamento
  const { data: agendamento, error } = await supabase
    .from('entrega_agendamentos')
    .insert([{ nome, cpf: cpfLimpo, email, telefone, empreendimento, unidade, data, horario, token: token || '' }])
    .select()
    .single()
  ...
  if (token) {
    await supabase.from('entrega_tokens').update({ usado: true }).eq('token', token)
  }
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      subject: 'Entrega de Chaves Confirmada - Markinvest',
      html: `<!DOCTYPE html>
<html>
  ...
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.10);">
      ...
      <tr><td style="background:#16a34a;padding:14px 32px;text-align:center;">
        <p style="color:#fff;font-size:13px;font-weight:700;margin:0;">&#10003; ENTREGA DE CHAVES CONFIRMADA</p>
      </td></tr>
      <tr><td style="background:#ffffff;padding:36px 32px;">
        <p style="color:#1B2F7E;font-size:22px;font-family:Georgia,serif;font-weight:400;margin:0 0 8px;">Ola, ${nome}!</p>
        <p style="color:#6b7280;font-size:14px;line-height:1.7;margin:0 0 28px;">Sua entrega de chaves foi agendada com sucesso. Confira os detalhes abaixo.</p>
        <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8f9ff;border-radius:12px;border:1px solid #e0e5f5;margin-bottom:24px;">
          <tr><td style="padding:16px 24px;border-bottom:1px solid #e0e5f5;">
            <p style="font-size:10px;font-weight:700;color:#1B2F7E;text-transform:uppercase;letter-spacing:0.12em;margin:0;">DETALHES DA ENTREGA</p>
          </td></tr>
    ...
    await transporter.sendMail({
      ...
      subject: 'Nova Entrega de Chaves - ' + empreendimento + ' | ' + dataCapitalizada,
      html: `<!DOCTYPE html>
<html>
<body style="margin:0;padding:0;background:#f0f4f8;font-family:'Segoe UI',Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f4f8;padding:30px 0;">
...
export async function PATCH(request) {
  ...
  const { error } = await supabase
    .from('entrega_agendamentos')
    .update({ status, motivo_cancelamento: motivo_cancelamento || '' })
    .eq('id', id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
